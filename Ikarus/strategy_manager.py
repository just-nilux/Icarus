...
class StrategyManager():
    ...
    async def source_plugin(self): 
        ...
        if not COLL_STR_MGR_PLUGIN in col_names:
            ...
            raise Exception(f'{COLL_STR_MGR_PLUGIN} collection does not exist')

        # Seperate the docs based on their type: resource_allocation, market_scan
        res_alloc = await self.mongo_cli.do_find(COLL_STR_MGR_PLUGIN,{'type':'resource_allocation'})
        
        if len(res_alloc) == 0:
            self.logger.debug(f'No new resource_allocation object in {COLL_STR_MGR_PLUGIN}')
            return
