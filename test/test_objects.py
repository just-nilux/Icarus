...
class test_order_from_dict(unittest.TestCase):
    def test_limit(self):
        order_data = {'price': 0.2906, 'amount': 4749.5664, 'quantity': 16344.0, 'fee': 0.0, 'orderId': 1652629743339, 'expire': 1559001600000}
        limit_from_dict = order_from_dict(order_data)
        limit_original = Limit(0.2906, 4749.5664, 16344.0, 0.0, 1652629743339, 1559001600000)
        self.assertEqual(limit_from_dict, limit_original)

    def test_market(self):
        order_data = {'price': 0.2906, 'amount': 4749.5664, 'quantity': 16344.0, 'fee': 0.0, 'orderId': 1652629743339}
        market_from_dict = order_from_dict(order_data)
        market_original = Market(0.2906, 4749.5664, 16344.0, 0.0, 1652629743339)
